...
src_val_dir = os.path.join(cfg.bdd_dir, 'det_20', 'det_val.json')
src_train_dir = os.path.join(cfg.bdd_dir, 'det_20', 'det_train.json')


os.makedirs(os.path.join(cfg.bdd_dir, 'det_20_coco'), exist_ok=True)

dst_val_dir = os.path.join(cfg.bdd_dir, 'det_20_coco', 'det_val_coco.json')
dst_train_dir = os.path.join(cfg.bdd_dir, 'det_20_coco', 'det_train_coco.json')
    ...
    try: # Avoid KeyError 
        for l in i['labels']:
          ...
          if l['category'] in id_dict.keys():
              ...
              #annotation["image_id"] = image['file_name']
